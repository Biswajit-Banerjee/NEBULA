...
import { Download, ZoomIn, ZoomOut, RotateCcw, Maximize, Minimize, Play, Pause, SkipForward, SkipBack } from 'lucide-react';
  ...
  const processData = (data, currentGen) => {
    ...
    data.forEach(reaction => {
      if (reaction.compound_generation) {
        Object.entries(reaction.compound_generation).forEach(([compound, gen]) => {
    ...
    data.forEach((reaction, index) => {
      try {
        ...
        const reactantNode = addUniqueNode(reactantNodeId, 'reaction-in', sourceGen, { 
        ...
        if (currentGen >= targetGen) {
          ...
          const productNode = addUniqueNode(productNodeId, 'reaction-out', targetGen, { 
            label: reaction.reaction,
            reaction: reaction
          });
          ...
          if (reaction.ec_list && reaction.ec_list.length > 0 && currentGen >= targetGen) {
            reaction.ec_list.forEach((ec, ecIndex) => {
              if (ec && ec !== 'N/A') {
                const ecNodeId = `ec_${ec}_${index}_${ecIndex}`;
  ...
  useEffect(() => {
    ...
    const link = g.append("g")
      ...
      .attr("stroke", d => {
        ...
        const gen = Math.min(d.generation || 0, genColors.length - 1);
        return genColors[gen].stroke;
      })
    ...
    node.filter(d => d.type === "reaction-in")
      .append("rect")
      .attr("width", 50)
      .attr("height", 25)
      .attr("x", -25)
    ...
    node.filter(d => d.type === "reaction-out")
      .append("rect")
      .attr("width", 50)
      .attr("height", 25)
      .attr("x", -25)
    ...
    node.filter(d => d.generation > 0)
      .append("text")
      .attr("x", d => d.type === "compound" ? 0 : (d.type === "ec" ? -20 : -15))
      .attr("y", d => d.type === "compound" ? -25 : (d.type === "ec" ? -15 : -20))
      .attr("text-anchor", "middle")
      .attr("font-size", 8)
    ...
    svg.call(zoom.transform, d3.zoomIdentity.scale(0.8).translate(width / 4, containerHeight / 4));
      ...
      <div className="mt-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between mb-2">
          <div className="flex items-center space-x-4 mb-2 md:mb-0">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={togglePlay}
                className={`p-1.5 ${isPlaying ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-green-50 text-green-600 hover:bg-green-100'} rounded-md`}
          ...
          <div className="flex flex-wrap gap-2">
            {generationColors.slice(0, Math.min(maxGeneration + 1, 6)).map((color, i) => (
        ...
        <div className="relative h-12 flex items-center">
          <input
            ...
            style={{
              background: `linear-gradient(to right, #3B82F6 0%, #3B82F6 ${(currentGeneration / maxGeneration) * 100}%, #E5E7EB ${(currentGeneration / maxGeneration) * 100}%, #E5E7EB 100%)`
            }}
          />
          ...
          <div className="absolute w-full flex justify-between top-5 px-2">
            {Array.from({ length: maxGeneration + 1 }).map((_, i) => (
              ...
              >
                <div 
                  className={`w-1 h-3 ${currentGeneration >= i ? 'bg-blue-500' : 'bg-gray-300'}`}
                ></div>
                <span className={`text-xs mt-1 ${currentGeneration === i ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>
                  {i}
                </span>
              </div>
            ))}
